  ...
  return (
    <section ref={ref} className={inView ? 'section-visible' : 'section-hidden'}>
      <div className={`flex ${ reversed ? "md:flex-row-reverse" : "md:flex-row"} w-full flex-col justify-center items-center mb-24 md:mb-32`}>
        <img src={image} alt={title} className={`flex-1 h-24 md:h-72 w-auto m-auto mb-10 md:mb-0 object-cover white-box-shadow ${ reversed ? "md:ml-8" : "md:mr-8"}`}/>
        <div className={`flex flex-col flex-1 h-full justify-center text-md md:text-lg ${ reversed ? "md:mr-8" : "md:ml-8"}`}>
